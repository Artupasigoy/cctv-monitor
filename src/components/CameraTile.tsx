  ...
  return (
    <div className={`tile${isFullscreen ? ' is-fullscreen' : ''}`} ref={fullscreenRef} onDoubleClick={handleDoubleClick}>
      <div className="tile-video-wrap">
        {!showPlaceholder ? (
          <video ref={videoRef} className="tile-video" autoPlay playsInline muted />
        ) : (
      ...
      <div className="tile-topbar">
        ...
        {isActive ? <StatusIndicator status={status.status} /> : <span className="status status-idle">IDLE</span>}
      </div>
